          ...
          if(v.type=='text'){
            ...
            switch (v.attribs.title) {
              ...
              case '周次(节次)':
                ...
                let {sections,weeks} = getSectionAndWeeks($(v).text().trim());
...
console.log(getSectionAndWeeks('2,7,12(周)[11-12节]'));
function getSectionAndWeeks(str) {
  ...
  if(res){
    //解析周次
    if(res[1].indexOf('-')>=0){
      let weeksInfo = res[1].split('-').map(v=>Number(v))
      for(let i=weeksInfo[0];i<=weeksInfo[1];i++){
        infos.weeks.push(i);
      }
    }else{
    ...
    if(res[2].indexOf('-')>=0){
      ...
      for(let i=secInfo[0];i<=secInfo[1];i++){
        if(i>=11){
          infos.sections.push({section:i-6});
        }else if(i>=5){
          infos.sections.push({section:i+2});
        }else{
          infos.sections.push({section:i});
        }
      }
    }else{
      res[2].split(',').forEach(v=>infos.sections.push({section:Number(v)}))
    }
  }
  // console.log(infos);
  return infos;
